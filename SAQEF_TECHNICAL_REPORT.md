...
## 29. Finding A fix — parallel function delivery (in progress, 2026-08-05)
...
**v3 status (4 function replicas, `N = 2 × cpu_count`, 2026-08-05):** median `cp_dynamic_share_pct` **15.87** (spread 15.58–16.13, tight across all 5 runs), throughput ~289 rps (≈2× v2, not 4× — consistent with the 2-core ceiling, not replica-starved). **The v2 sentence above claiming the busy-spin ratio is "scheduling-order-invariant as predicted" is SUPERSEDED — it is not.** Going 1→4 replicas moved the headline 11.1 → 15.87, a ~43% relative shift. The reviewer's causal story for this (serialization making CP "do more work per invocation") is **contradicted by the data's direction**: it predicts the share should have *dropped* when serialization was removed, and it rose. The defensible reading: under 1 replica, 20 concurrent requests queue on one GIL-bound process and the CP components (gateway/queue-worker/nats) spend most of the window **idle** — per-wall-second CP busy went ~0.089 → ~0.27 (≈3×) from the replica fix, i.e. CP busy-time tracks *active concurrency*, not queue length. The 1-replica 11.1 therefore **understated** CP's contribution; 15.87 is the protocol-conformant number. Corrected comparison: Fn **23.59–24.59** vs OpenFaaS **15.87** → gap ≈ **7.7–8.7 pp > 5 pp gate**, still discriminating; the uncorrected ~13.5 pp figure would have overstated the platform difference by >50%.

**v3 caveat — host plausibility (fixed in harness v9.10):** run_2 of v3 tripped `host_plausible: false` (`host_cpu_sec` 23.23 vs ceiling 20.78 = 2 cores × 10.39 s wall, sat 112%) — the first host-ceiling break in the project, on the *shortest* run yet. Root cause: the host `/proc/stat` counter is read a few ms **outside** the load window; that fixed edge-slop is a constant absolute fraction, so it is a larger percentage of a 10 s window than of a 21 s/75 s one (previous sessions read 99–104%). Since busy ticks over a window `W` can never exceed `cpu_count × W`, the harness now defines `host_saturation_pct`/`host_plausible` against the host's **own** sampling window (`host_window_s = t_host_after − t_host_before`), which is structurally self-consistent — the gate can only trip on a real CPU-count/counter anomaly now, never on fast-run window alignment. `host_window_s` is exposed per run for audit; container-side `cpu_sec_ceiling` is unchanged (load window). The OpenFaaS `gates` table also regained the host columns (`host_cpu_s`, `host_sat%`, `host_plausible`) with a hard "HOST IMPLAUSIBLE — do not cite host metrics" marker when false, so this class of failure cannot be committed silently again.

**Remaining before sign-off:** rerun v3 (4 replicas) on the v9.10 harness — expect `host_plausible: true`, sat ≈ 100% regardless of window length, delta% ≈ 0, 6/6 mapped — then one confirmatory run at 8 replicas (`SAQEF_REPEAT=1 SAQEF_REPLICAS=8` → `_quick`) to show the share holds near 15.87 and the replica choice is insensitive past the 2-core ceiling.
