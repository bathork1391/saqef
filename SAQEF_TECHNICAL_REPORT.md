...
**(a) hey reinstall gate was size-only.** The old check reused any `hey` on PATH with `stat -c%s ≥ 1000`. That cannot tell a real binary from a truncated 403-HTML page or a wrong binary of ≥1000 bytes — which is exactly the class of failure that produced the v9.7 hey saga (rc=0, stdout `json`, no valid report ever). **Fix:** `hey_smoke_ok()` runs the candidate with `-n 2 -c 1 -o json http://localhost:8080/` and parses stdout as JSON. Pass → reuse; fail → wipe every copy (`/go/bin`, `/usr/local/bin`) and reinstall (go install, then binary-download fallback), re-smoke-testing the replacement. A still-broken install prints a warning and the Python generator fallback carries the run (`loadgen_fallback` stays truthful).
